...
class TestTaskComplexityEstimator:
    def test_short_tasks_return_min_turns(self) -> None:
        """Short descriptions all return MIN_TURNS=15 (chars < 300)."""
        estimator = TaskComplexityEstimator()
        assert estimator.estimate_turns("implement login") == 15
        assert estimator.estimate_turns("debug crash") == 15
        assert estimator.estimate_turns("analyze report") == 15
        assert estimator.estimate_turns("refactor code") == 15
        assert estimator.estimate_turns("") == 15

    def test_estimate_scales_with_length(self) -> None:
        """Turns = chars // 20, clamped to [15, 60]."""
        estimator = TaskComplexityEstimator()
        # 325 chars → 325 // 20 = 16
        assert estimator.estimate_turns("x" * 325) == 16
        # 700 chars → 700 // 20 = 35
        assert estimator.estimate_turns("x" * 700) == 35

    def test_estimate_capped_at_max(self) -> None:
        """Very long descriptions are capped at MAX_TURNS=60."""
        estimator = TaskComplexityEstimator()
        # 2000 chars → 2000 // 20 = 100 → capped at 60
        assert estimator.estimate_turns("x" * 2000) == 60

    def test_file_count_ignored(self) -> None:
        """file_count is accepted for backward compat but does not affect result."""
        estimator = TaskComplexityEstimator()
        desc = "x" * 350
        # 350 // 20 = 17 (same with or without file_count)
        assert estimator.estimate_turns(desc) == 17
        assert estimator.estimate_turns(desc, file_count=10) == 17

    def test_history_adaptive_adjustment(self) -> None:
        """When history is available, estimates are adjusted ±25%."""
        estimator = TaskComplexityEstimator()
        # Add history where actual turns were much lower than estimate
        # estimate for "a"*400 → 400//20=20; record actual=10 (lower)
        estimator.add_history("a" * 400, actual_turns=10)
        # With history, new estimate should be lower
        adjusted = estimator.estimate_turns("b" * 400)
        # Base is 20, ratio=10/20=0.5 → clamped to 0.75 → 20*0.75=15
        assert adjusted == 15
