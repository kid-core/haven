...
# MCP tools that write/modify — require confirmation
_MCP_WRITE_TOOLS: frozenset[str] = frozenset({
    "index_repository",   # writes to DB + reads whole repo
    "delete_project",     # destructive
    "manage_adr",         # creates/modifies ADRs
    "ingest_traces",      # writes traces
})
...
async def discover_tools(client: MCPClient, server_name: str) -> list[ToolSpec]:
    ...
    - Only write/destructive tools require confirmation; read-only tools pass through.
    ...
    for raw in tools:
        ...
        # Long-running tools need extended timeout (e.g. indexing)
        tool_timeout = 600.0 if name == "index_repository" else 30.0
        needs_confirm = name in _MCP_WRITE_TOOLS
        spec = ToolSpec(
            name=prefixed,
            description=desc,
            ...
            policy=ToolPolicy(require_confirm=needs_confirm, timeout=tool_timeout, rate_limit=10.0),
        )
        specs.append(spec)
