...
class Router(BaseReActLoop):
    ...
    def __init__(
        ...
        self._estimator = complexity_estimator
        ...
        from tools.dag_task import set_spawn_manager as set_dag_spawn
        set_dag_spawn(self._spawn_manager)
        ...
        set_background_context(
            ...
            complexity_estimator=self._estimator,
        )
        ...
        # ── User confirmation bridge ─────────────────────────────
        self._confirm_store = ConfirmationStore()
        self._current_channel_id: str = ""

    def set_current_channel(self, channel_id: str) -> None:
        """Set the current channel id for confirmation routing."""
        self._current_channel_id = channel_id

    def get_confirm_store(self) -> ConfirmationStore:
        """Return the confirmation store for transport integration."""
    ...
    async def _pre_tool_check(self, name: str, arguments: dict) -> dict | None:
        """Check if tool requires user confirmation; sends confirm request via transport."""
        if self._registry.is_confirm_required(name):
            import json
            from core.pending_file import current_session
            session_id = current_session() or "default"
            channel_id = self._current_channel_id or ""

            if channel_id:
                approved = await self._confirm_store.request(
                ...
                if approved:
                    logger.info("Tool %r confirmed by user", name)
                    return None
                else:
                    logger.info("Tool %r denied by user", name)
                    return {
                        "role": "tool",
                        "tool_call_id": "",
                        "content": json.dumps({
                            "status": "denied",
                            "reason": "User denied the confirmation request.",
                        }),
                    }
            else:
                logger.info("Tool %r requires confirmation but no channel — auto-approving", name)
                return None
        return None
    ...
    async def process(
        ...
        ``max_turns``:
          - ``None`` (default) → auto-estimate via TaskComplexityEstimator
          - ``int`` → manual override, used as-is
        """
        # ── 決定 max_turns ──
        if max_turns is None:
            if self._estimator is not None:
                max_turns = self._estimator.estimate_turns(user_message)
            else:
        ...
        # Phase 2: Heartbeat tracker for lost-agent detection
        ...
        for _turn in range(max_turns):
            ...
            # ── Phase 2: Progress heartbeat check ──
            if heartbeat.should_check(turn_count):
                messages.append({
                    "role": "system",
                    "content": heartbeat.heartbeat_prompt(),
                })
                logger.debug("Heartbeat prompt injected at turn %d", turn_count)
            ...
            # Phase 2: Record heartbeat self-assessment if present
            if content and heartbeat.should_check(turn_count):
                heartbeat_triggered, hb_reason = heartbeat.record(content, turn_count)
                if heartbeat_triggered:
                    logger.warning(
                        "Heartbeat lost-agent detected at turn %d: %s",
                        turn_count, hb_reason,
                    )
                    self._save_history(session_id, messages)
                    self._tracer.log_summary()
                    return (
                        "[heartbeat] Agent appears stuck — "
                        f"{hb_reason}. Consider spawning a fresh agent."
                    )
            ...
            if not tool_calls:
                ...
                if self._budget_tracker is not None:
                    logger.info("Budget after session: %s", self._budget_tracker.summary())
                return text
        ...
        # ── 回饋校正 ──
        if self._estimator is not None:
            self._estimator.add_history(user_message, turn_count)
    ...
    async def _execute_tool(
        ...
        pre = await self._pre_tool_check(name, arguments)
        if pre is not None:
            return pre
