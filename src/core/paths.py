...
environment variables. When neither is set, paths match the
WSL2 layout (``/mnt/z/haven``, ``/mnt/z/Core``).
...
def code_dir() -> Path:
    """Haven 程式碼目錄 ``$HAVEN_ROOT/src``."""
    return haven_dir() / "src"
...
def openclaw_env() -> Path:
    """OpenClaw Gateway env file. (Deprecated: use haven_env for local config)"""
    return Path(os.getenv("OPENCLAW_ENV", "/mnt/z/haven/.env"))
...
def identity_file() -> Path:
    """KID 身份檔."""
    return haven_dir() / "soul" / "IDENTITY.md"


def soul_file() -> Path:
    """靈魂設定檔."""
    return haven_dir() / "soul" / "SOUL.md"


def user_file() -> Path:
    """使用者資訊檔."""
    return haven_dir() / "soul" / "USER.md"


def memory_file() -> Path:
    """長期記憶檔."""
    return haven_dir() / "soul" / "MEMORY.md"


def agents_file() -> Path:
    """行為規則檔."""
    return haven_dir() / "soul" / "AGENTS.md"


def chronicle_file() -> Path:
    """靈魂編年史."""
    return haven_dir() / "soul" / "CHRONICLE.md"
